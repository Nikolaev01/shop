...
abstract class AbstractObjectDB{
    ...
    private $id = null;//
    private $properties = array();//Свойства объектов

    protected $table_name = "";//Объект определенной таблицы
    //Возвращаем значения
    public function __construct($table_name, $formate_date)
    {
        $this->table_name = $table_name;
        $this->format_date = $formate_date;
    }
    //Метод установки БД
    public static function setDB($db){
        self::$db = $db;
    }
    //Загрузка объектов по ID
    ...
    //метод проверки сохранен ли данный объект
    ...
    //метод сохранения в базу данных изменения
           ...
           switch ($value["type"]){
               case self::TYPE_TIMESTAMP:
                   if (!is_null($value["value"]))
                       $value["value"] = strftime($value["value"]);
                   break;
               case self::TYPE_IP:
    ...
    //Метод формирования массив объектов
    ...
    //Метод отвечающий за доступ ко всем свойствам
